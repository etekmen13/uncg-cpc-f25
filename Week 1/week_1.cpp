...
void prefix_sum() {
  ...
  for (int i = 0; i < vec.size(); ++i)
    prefix[i + 1] =
        prefix[i] + vec[i]; // next accumulated value depends on current
...
/*
  ...
  - When 'out' is empty and we need to pop-left, we transfer all from 'in' to
  'out' (recomputing running max for 'out'). Amortized O(1) per operation.
...
struct MaxQueue {
  ...
  // Move everything from 'in' to 'out', reversing order and rebuilding running
  // max for 'out'.
  void transfer() {
    while (!in.empty()) {
      int x = in.pop();
...
// MaxQueue aka Sliding Window Aggregation (SWAG) aka folding over a monoid
// (associative op, identity element);
vector<int> sliding_window_max_queue() {
  ...
  for (int i = 0; i < (int)nums.size(); ++i) {
    ...
    // First full window ends at i == k-1. After that, record max then pop the
    // leftmost.
    if (i >= k - 1) {
...
void two_pointer_under_budget() {
  ...
  for (int l = 0; l < vec.size();
       sum -= vec[l++]) // before every l++ subtract vec[l] from our budget
  {
    while (r + 1 < vec.size() && sum + vec[r + 1] <= t)
      sum += vec[++r];         // after ++r, add vec[r] top our budget
...
// two pointer to calculate the longest contiguous segment with at most K
// distinct elements
void two_pointer_at_most_k_distinct() {
  ...
  cout << a << endl;
  cout << "longest subarray with at most " << K
       << " distinct numbers is of length: " << best << endl;
}
#pragma endregion
...
void sort_custom() {
  vector<pair<string, string>> names{
      {"Emre", "Tekmen"},       {"Sasha", "Steffey"},
      {"Yeraldine", "Tamayo"},  {"John", "Smith"},
      {"Eric", "Smith"},        {"Kendrick", "Lamar"},
